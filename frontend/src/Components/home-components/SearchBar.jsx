  ...
  <div className="flex flex-col lg:flex-row gap-4 items-center justify-between bg-white p-4 rounded-3xl shadow-sm border border-slate-100">
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 flex-grow w-full">
      ...
      <button
        ...
        disabled={!userCoords}
        className={`px-4 py-3 rounded-2xl text-xs font-black uppercase transition-all flex items-center justify-center gap-2 ${isSortingNearest ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'} ${!userCoords && 'opacity-50 cursor-not-allowed'}`}
      >
